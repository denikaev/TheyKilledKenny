...
class ViewController: UIViewController, CAAnimationDelegate {
    ...
    fileprivate var boardSide: CGFloat = 0.0
    fileprivate  var gameBoardView = UIView()
    
    fileprivate var piece = Piece()
    ...
    /// Создаем вьюшку поля
    func createViewsOnBoard() {
        
        let _boardSide = self.view.bounds.size.width - 20
        let boardCenter = self.view.center.y
        let gameBoardViewRect = CGRect(x: 10, y: boardCenter - (_boardSide / 2), width: _boardSide, height: _boardSide)
        let _gameBoardView = UIView(frame: gameBoardViewRect)
        _gameBoardView.layer.cornerRadius = 10
        _gameBoardView.layer.borderWidth = 1
        _gameBoardView.layer.borderColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)
        
        self.boardSide = _boardSide
        self.gameBoardView = _gameBoardView
    ...
    func createViewForPiece(count: Int) {
        ...
        for i in 1...count {
            ...
            self.piece.gamePieceRect = gamePieceRect
            self.piece.gamePieceView = gamePieceView
            
            //Тэг фишки
            self.piece.gamePieceView.tag = i
            self.piece.index = gamePieceView.tag
            
            // gamePieceView.backgroundColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)
            self.piece.gamePieceView.image = UIImage(named: "Button")
            ...
            self.piece.gamePieceView.contentMode = .scaleAspectFit
            ...
            gameBoardView.addSubview(self.piece.gamePieceView)
            ...
            self.piece.gamePieceView.addSubview(numberLabel)
        }
    }
